...
public class FogDevice extends PowerDatacenter {
    ...
    private List<String> actuatorTypes = new ArrayList<>();
    
    public void addActuator(String actuatorType) {
        if (!actuatorTypes.contains(actuatorType)) {
    ...
    public void addNetworkUsage(double amount) {
        this.totalNetworkUsage += amount;
        //System.out.println("📦 [DEBUG] " + getName() + " → 누적: " + this.totalNetworkUsage);
    }
    ...
    protected void moduleSend(SimEvent ev) {
        ...
        NetworkUsageMonitor.sendingModule(getId(), (double) object.get("delay"), appModule.getSize());
    ...
    protected void sendTupleToActuator(Tuple tuple) {
        ...
        for (Pair<Integer, Double> actuatorAssociation : getAssociatedActuatorIds()) {
            ...
            if (tuple.getDestModuleName().equals(actuatorType)) {
            	// ✅ 네트워크 사용량 기록
                //TimeKeeper.getInstance().addNetworkUsage(getId(), tuple.getCloudletFileSize());
                send(actuatorId, delay, FogEvents.TUPLE_ARRIVAL, tuple);
                //this.addNetworkUsage(tuple.getCloudletFileSize());
                return;
            }
        }
    ...
    protected void updateTimingsOnReceipt(Tuple tuple) {
    	//System.out.println("📦 [DEBUG] 수신된 튜플 → src=" + tuple.getSrcModuleName() + ", dest=" + tuple.getDestModuleName());
        ...
        for (AppLoop loop : loops) {
            if (loop.hasEdge(srcModule, destModule) && loop.isEndModule(destModule)) {
                ...
                TimeKeeper.getInstance().getEndTimes().put(tupleId, CloudSim.clock());
                if (startTime == null) {
                    // System.out.println("⚠️ [DEBUG] loop 종료지만 emitTime 없음 → tupleId=" + tupleId);
                    break;
                }
             ...
             // ✅ emitTime 제거하고 endTime 기록
                TimeKeeper.getInstance().getEmitTimes().remove(tupleId);
                TimeKeeper.getInstance().getEndTimes().put(tupleId, CloudSim.clock());


                double currentAverage = TimeKeeper.getInstance()
                    .getLoopIdToCurrentAverage().getOrDefault(loop.getLoopId(), 0.0);
                int currentCount = TimeKeeper.getInstance()
                    .getLoopIdToCurrentNum().getOrDefault(loop.getLoopId(), 0);

                double newAverage = (currentAverage * currentCount + delay) / (currentCount + 1);
                TimeKeeper.getInstance().getLoopIdToCurrentAverage().put(loop.getLoopId(), newAverage);
                TimeKeeper.getInstance().getLoopIdToCurrentNum().put(loop.getLoopId(), currentCount + 1);

                //System.out.println("✅ [DEBUG] 루프 종료: tupleId=" + tupleId + ", delay=" + delay);
                break;
            }
        }
     // ✅ DISPLAY 모듈 도착 시 endTime 기록 (loop에 포함되지 않더라도)
       // String destModule = tuple.getDestModuleName();
        if (destModule != null && destModule.toUpperCase().contains("DISPLAY")) {
            int tupleId = tuple.getActualTupleId();
            if (tupleId != -1 && TimeKeeper.getInstance().getEmitTimes().containsKey(tupleId)) {
                TimeKeeper.getInstance().getEndTimes().put(tupleId, CloudSim.clock());
               // System.out.println("✅ [DEBUG] DISPLAY 도착 → endTime 등록: " + tupleId);
            } else {
    ...
    protected void sendUpFreeLink(Tuple tuple) {
        ...
        NetworkUsageMonitor.sendingTuple(getId(), getUplinkLatency(), tuple.getCloudletFileSize());
    ...
    protected void sendDownFreeLink(Tuple tuple, int childId) {
        ...
        NetworkUsageMonitor.sendingTuple(getId(), latency, tuple.getCloudletFileSize());
